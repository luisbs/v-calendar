...
import { reactive, computed, openBlock, createBlock, Transition, withCtx, renderSlot, h, createElementBlock, createElementVNode, normalizeStyle, normalizeClass, createCommentVNode, createTextVNode, toDisplayString, resolveComponent, createVNode, Fragment, renderList, mergeProps, withModifiers, inject, toRef, useSlots as useSlots$1, ref, defineComponent, watch, onMounted, unref, nextTick, normalizeProps, guardReactiveProps } from "vue";
...
if (DataView$1 && getTag(new DataView$1(new ArrayBuffer(1))) != dataViewTag$2 || Map$1 && getTag(new Map$1()) != mapTag$4 || Promise$2 && getTag(Promise$2.resolve()) != promiseTag || Set$2 && getTag(new Set$2()) != setTag$4 || WeakMap$1 && getTag(new WeakMap$1()) != weakMapTag$1) {
...
const has = has$1;
const hasAny = (obj, props) => some$1(props, (p) => has$1(obj, p));
const some = some$1;
...
toPairs$1(locales).forEach(([id, { dow, L }]) => {
  locales[id] = {
    id,
    firstDayOfWeek: dow,
    masks: { L }
...
const computedLocales = computed(() => {
  return mapValues(state.locales, (v) => {
    v.masks = defaultsDeep$1(v.masks, state.masks);
    return v;
  });
});
...
var screensPlugin = {
  install: (app, screens) => {
    screens = defaultsDeep$1(screens, window && window.__screens__, defaultScreens);
    ...
    const refreshMatches = () => {
      state2.matches = toPairs$1(state2.queries).filter((p) => p[1].matches).map((p) => p[0]);
    };
    const refreshQueries = () => {
      if (!shouldRefreshQueries || !window || !window.matchMedia)
        return;
      state2.queries = mapValues(screens, (v) => {
    ...
    app.mixin({
      ...
      computed: {
        $screens() {
          return (config, def) => state2.matches.reduce((prev, curr) => has(config, curr) ? config[curr] : prev, isUndefined(def) ? config.default : def);
        }
      }
    });
  }
...
const arrayHasItems = (array) => isArrayLikeObject(array) && array.length > 0;
...
var CustomTransition = /* @__PURE__ */ _export_sfc(_sfc_main$i, [["render", _sfc_render$5]]);
const _sfc_main$h = {
  name: "Popover",
  ...
  computed: {
    content() {
      return typeof this.$slots.default === "function" && this.$slots.default({
        direction: this.direction,
  ...
  methods: {
    ...
    show(opts = {}) {
      const _a = opts, { id } = _a, _opts = __objRest(_a, ["id"]);
      opts.action = "show";
    ...
    update(_a = {}) {
      var _b = _a, { id } = _b, opts = __objRest(_b, ["id"]);
...
class Theme {
  ...
  normalizeAttr({ config, type }) {
    ...
    defaults$1(root2, { start: root2.startEnd, end: root2.startEnd }, normAttr);
    toPairs$1(root2).forEach(([targetType, targetConfig]) => {
      ...
      if (!has(root2, `${targetType}.color`)) {
        set(root2, `${targetType}.color`, targetColor);
      }
    });
    return root2;
  }
  normalizeHighlight(config) {
    ...
    toPairs$1(highlight).forEach(([_, targetConfig]) => {
      const c = defaults$1(targetConfig, {
        isDark: this.isDark,
        color: this.color
      });
  ...
  normalizeNonHighlight(type, config, styleFn) {
    ...
    toPairs$1(attr).forEach(([_, targetConfig]) => {
      defaults$1(targetConfig, { isDark: this.isDark, color: this.color });
...
class DateInfo {
  constructor(config, { order = 0, locale, isFullDay } = {}) {
    ...
    if (config.on) {
      const or = (isArrayLikeObject(config.on) ? config.on : [config.on]).map((o) => {
        if (isFunction(o))
          return o;
  ...
  static get patterns() {
    return {
      ...
      days: {
        validate: (days) => isArrayLikeObject(days) ? days : [parseInt(days, 10)],
        test: (day, days) => days.includes(day.day) || days.includes(-day.dayFromEnd)
      },
      weekdays: {
        validate: (weekdays2) => isArrayLikeObject(weekdays2) ? weekdays2 : [parseInt(weekdays2, 10)],
      ...
      ordinalWeekdays: {
        validate: (ordinalWeekdays) => Object.keys(ordinalWeekdays).reduce((obj, ck) => {
          ...
          if (!weekdays2)
            return obj;
          obj[ck] = isArrayLikeObject(weekdays2) ? weekdays2 : [parseInt(weekdays2, 10)];
          return obj;
        }, {}),
      ...
      weeks: {
        validate: (weeks) => isArrayLikeObject(weeks) ? weeks : [parseInt(weeks, 10)],
        test: (day, weeks) => weeks.includes(day.week) || weeks.includes(-day.weekFromEnd)
      },
      months: {
        validate: (months) => isArrayLikeObject(months) ? months : [parseInt(months, 10)],
        test: (day, months) => months.includes(day.month)
      },
      years: {
        validate: (years) => isArrayLikeObject(years) ? years : [parseInt(years, 10)],
...
function resolveConfig(config, locales2) {
  ...
  config = isObject(config) ? defaultsDeep$1(config, defLocale) : defLocale;
  return config;
}
class Locale {
  ...
  normalizeMasks(masks2) {
    return (arrayHasItems(masks2) && masks2 || [
      isString(masks2) && masks2 || "YYYY-MM-DD"
  ...
  normalizeDate(d, config = {}) {
    ...
    if (result && patch) {
      ...
      const parts = __spreadValues(__spreadValues({}, this.getDateParts(fillDate)), pick$1(this.getDateParts(result), PATCH_KEYS[patch]));
  ...
  hourIsValid(hour, validHours, dateParts) {
    if (!validHours)
      return true;
    if (isArrayLikeObject(validHours))
...
const _hoisted_1$6 = { class: "vc-day-popover-row" };
const _hoisted_2$5 = {
  key: 0,
...
function _sfc_render$4(_ctx, _cache, $props, $setup, $data, $options) {
  return openBlock(), createElementBlock("div", _hoisted_1$6, [
    $options.indicator ? (openBlock(), createElementBlock("div", _hoisted_2$5, [
...
var PopoverRow = /* @__PURE__ */ _export_sfc(_sfc_main$g, [["render", _sfc_render$4]]);
const _defSize = "26px";
...
function _sfc_render$3(_ctx, _cache, $props, $setup, $data, $options) {
  return openBlock(), createElementBlock("svg", {
    ...
    createElementVNode("path", { d: $data.path }, null, 8, _hoisted_2$4)
  ], 8, _hoisted_1$5);
}
var SvgIcon = /* @__PURE__ */ _export_sfc(_sfc_main$f, [["render", _sfc_render$3]]);
...
const _hoisted_2$3 = { class: "vc-nav-header" };
...
var CalendarNav = /* @__PURE__ */ _export_sfc(_sfc_main$e, [["render", _sfc_render$2]]);
  ...
  methods: {
    ...
    refreshPopovers() {
      ...
      if (arrayHasItems(this.popovers)) {
        popoverEvents = getPopoverTriggerEvents(defaults$1({ id: this.dayPopoverId, data: this.day, isRenderFn: true }, ...this.popovers));
      }
...
const _sfc_main$c = {
  ...
  render() {
    ...
    this.page.days.forEach((day, i) => {
      ...
      dayCells.push(h(_sfc_main$d, __spreadProps(__spreadValues({}, this.$attrs), {
        day
      }), this.$slots));
...
const _sfc_main$b = {
  ...
  render() {
    const panes = this.pages.map((page, i) => {
      ...
      const params = __spreadProps(__spreadValues({}, this.$attrs), {
        key: page.key,
        page,
        position,
        row,
      ...
      return this.safeSlot("calendar-panel", params) || h(_sfc_main$c, params, this.$slots);
    });
    ...
    const getNavPopover = () => h(_sfc_main$h, {
      id: this.sharedState.navPopoverId,
      contentClass: "vc-nav-popover-container",
      ref: "navPopover"
    }, {
      default: ({ data: data2 }) => {
        const { popover, position, page } = data2;
        return this.safeSlot("calendar-popover", {
          popover,
          position,
          modelValue: page,
    ...
    const getDayPopover = () => h(_sfc_main$h, {
      ...
      pages.forEach((p) => {
        p.days.forEach((d) => {
          ...
          if (reset) {
            ...
            map2 = omit$1(d.attributesMap, deletes);
            shouldRefresh = true;
          } else {
...
const _hoisted_1$3 = { class: "vc-select" };
...
var TimeSelect = /* @__PURE__ */ _export_sfc(_sfc_main$a, [["render", _sfc_render$1]]);
const _amOptions = [
  { value: 0, label: "12" },
  { value: 1, label: "1" },
  { value: 2, label: "2" },
...
const _hoisted_2$1 = { class: "vc-time-content" };
const _hoisted_3 = {
  key: 0,
  class: "vc-time-date"
};
...
var TimePicker = /* @__PURE__ */ _export_sfc(_sfc_main$9, [["render", _sfc_render]]);
const _baseConfig = {
  type: "auto",
  mask: "iso",
  timeAdjust: ""
...
const _sfc_main$8 = {
  ...
  render() {
    ...
    const calendar = () => h(_sfc_main$b, __spreadProps(__spreadValues({}, this.$attrs), {
  ...
  computed: {
    ...
    popover_() {
      return defaultsDeep$1(this.popover, getDefault("datePicker.popover"));
    },
    selectAttribute_() {
...
function useCommons() {
  const state2 = inject("sharedState");
  const navPopoverId = toRef(state2, "navPopoverId");
  const dayPopoverId = toRef(state2, "dayPopoverId");
  const format = (date, mask) => {
    return state2.locale.format(date, state2.masks[mask]);
  };
  const pageFromDate = (date) => {
    return state2.locale.getDateParts(state2.locale.normalizeDate(date));
  };
...
function serializeListeners(...sources) {
  return sources.reduce((result, source) => {
    var _a;
    for (const [k, cb] of Object.entries(source)) {
      const key = k.startsWith("on") ? k : "on" + (((_a = k.at(0)) == null ? void 0 : _a.toUpperCase()) || "") + k.substring(1);
...
function appendPopoverEvents(attribute, events) {
  const { key, customData, popover } = attribute;
  if (!popover)
    return;
  const params = [
  ...
  events.unshift(params.reduce((prev, current) => __spreadValues(__spreadValues({}, current), prev), {}));
}
function appendContentStyles({ key, highlight }, { isDate: isDate2, isComplex, onStart, onEnd }, styles) {
  if (!highlight)
    return;
  const { base, start, end } = highlight;
  if (isDate2 || isComplex || onStart) {
    styles.push({
      key: `${key}-content`,
      class: start.contentClass,
      style: start.contentStyle
...
function appendBackgroundStyles({ key, highlight }, { isDate: isDate2, isComplex, onStart, onEnd }, styles) {
  if (!highlight)
    return;
  const { base, start, end } = highlight;
  if (isDate2 || isComplex || onStart && onEnd) {
    styles.push({
      key,
      wrapperClass: "vc-day-layer vc-day-box-center-center",
      class: ["vc-highlight", start.class],
...
function appendStyles(attr, item, { isDate: isDate2, onStart, onEnd }, styles) {
  if (!attr[item])
    return;
  const { key } = attr;
  const className = `vc-${item}`;
  const { base, start, end } = attr[item];
  if (isDate2 || onStart) {
    styles.push({
      key,
      class: [className, start.class],
...
function serializeDayStyles(day) {
  const options = {
    popovers: [],
    backgrounds: [],
    content: [],
  ...
  day.attributes.forEach((attr) => {
    const { isDate: isDate2, isComplex, startTime, endTime } = attr.targetDate;
    const info = {
      isDate: isDate2,
      isComplex,
...
function dispatchEvent(name, options) {
  document == null ? void 0 : document.dispatchEvent(new CustomEvent(name, options));
}
function serializePopoverEvents(opts) {
  const { visibility } = opts;
  ...
  let hovered = false;
  let focused = false;
  const preparePopoverOptions = (target, forceUpdate = false) => {
    const popover = (target == null ? void 0 : target.dataset.popover) || "";
    const sameItem = lastPopover === popover;
    lastPopover = popover;
    opts.ref = target;
    opts.data = __spreadProps(__spreadValues({}, opts.data), { popover });
  ...
  return {
    ...
    onFocusout(ev) {
      if (useFocus && focused && !elementContains(ev.currentTarget, ev.relatedTarget)) {
        focused = false;
        if (!useHover || !hovered) {
          preparePopoverOptions(ev.currentTarget);
          dispatchEvent("hide-popover", { detail: opts });
...
function useSlots() {
  ...
  const callSlot = (name, args, fallback) => {
    var _a;
    return hasSlot(name) ? (_a = slots[name]) == null ? void 0 : _a.call(slots, args) : fallback;
  };
  const createSlot = (name, args, fallback) => {
    if (hasSlot(name))
      return () => {
        var _a;
        return (_a = slots[name]) == null ? void 0 : _a.call(slots, args);
      };
...
function useRef(initial, handler) {
  const currentValue = ref(initial);
  return [
    currentValue,
    (newValue) => {
      currentValue.value = handler ? handler(newValue, currentValue.value) : newValue;
    }
  ];
}
const __default__$5 = {
  ...
  render() {
    ...
    const layers = [];
    if ((_a = this.options.backgrounds) == null ? void 0 : _a.length) {
      const backgrounds = this.options.backgrounds.map(({ key, class: backgroundClass, wrapperClass, style }) => {
        return h("div", { key, class: wrapperClass }, [
          h("div", { class: backgroundClass, style })
        ]);
      });
      layers.push(h("div", { class: "vc-highlights vc-day-layer" }, backgrounds));
    }
    if (this.hasSlot("day-content")) {
      layers.push(this.callSlot("day-content", {
        day: this.day,
        props: dayContentProps,
        events: this.dayEvents
      }));
    } else {
      const dayContent = (_b = this.options.content) == null ? void 0 : _b.at(-1);
      const dayContentClass = [
        "vc-day-content vc-focusable",
        this.day.isDisabled && "is-disabled",
        (dayContent == null ? void 0 : dayContent.class) || ""
      ];
      const params = __spreadValues(__spreadProps(__spreadValues({}, dayContentProps), {
        ref: this.dayContent,
        class: dayContentClass,
        style: dayContent == null ? void 0 : dayContent.style
      }), this.dayEvents);
      layers.push(h("span", params, this.day.label));
    }
    if ((_c = this.options.dots) == null ? void 0 : _c.length) {
      const dots = this.options.dots.map((_d) => {
        var _e = _d, { wrapperClass } = _e, args = __objRest(_e, ["wrapperClass"]);
        return h("span", args);
      });
      layers.push(h("div", { class: "vc-day-layer vc-day-box-center-bottom" }, h("div", { class: "vc-dots" }, dots)));
    }
    if ((_f = this.options.bars) == null ? void 0 : _f.length) {
      const bars = this.options.bars.map((_g) => {
        var _h = _g, { wrapperClass } = _h, args = __objRest(_h, ["wrapperClass"]);
        return h("span", args);
      });
      layers.push(h("div", { class: "vc-day-layer vc-day-box-center-bottom" }, h("div", { class: "vc-bars" }, bars)));
    }
...
const _sfc_main$7 = /* @__PURE__ */ defineComponent(__spreadProps(__spreadValues({}, __default__$5), {
  ...
  setup(__props, { expose, emit }) {
    ...
    const newEvent = (name) => (ev) => {
      const params = __spreadProps(__spreadValues({}, props.day), {
        el: dayContent.value,
    ...
    const refresh = () => {
      if (!props.day.shouldRefresh)
        return;
      props.day.shouldRefresh = false;
      props.day.attributes = Object.values(props.day.attributesMap || {}).sort((a, b) => a.order - b.order);
      Object.assign(options, serializeDayStyles(props.day));
    };
    const refreshEvents = () => {
      var _a;
      if (!((_a = options.popovers) == null ? void 0 : _a.length)) {
        dayEvents.value = baseEvents;
        return;
      }
      const popoverParams = options.popovers.reduce((prev, current) => __spreadValues(__spreadValues({}, current), prev), { id: dayPopoverId.value, data: props.day });
    ...
    return { props, emit, hasSlot, callSlot, dayPopoverId, theme, dayContent, dayEvents, options, newEvent, baseEvents, refresh, refreshEvents };
  }
}));
function useWeekdays() {
  const { format, locale } = useCommons();
  const daysInWeek2 = computed(() => locale.daysInWeek);
  const weekdayLabels = computed(() => {
    return locale.getWeekdayDates().map((date) => format(date, "weekdays"));
  });
  return { daysInWeek: daysInWeek2, weekdayLabels };
}
const __default__$4 = {
  inheritAttrs: false,
  render() {
    ...
    const weekClass = this.weeknumbers.side ? ["vc-weeks", "vc-show-weeknumbers", `is-${this.weeknumbers.side}`] : ["vc-weeks"];
    const weeknumberClass = [
      "vc-weeknumber-content",
      "is-" + ((_a = this.weeknumbers.side) != null ? _a : "left") + (this.weeknumbers.outside ? "-outside" : "")
    ];
    const renderWeeknumberCell = (weeknumber) => {
      const onClick = (ev) => this.onClick(ev, weeknumber);
...
const _sfc_main$6 = /* @__PURE__ */ defineComponent(__spreadProps(__spreadValues({}, __default__$4), {
  ...
  setup(__props, { expose, emit }) {
    ...
    const { daysInWeek: daysInWeek2, weekdayLabels } = useWeekdays();
    const weeknumbersKey = computed(() => {
      return props.weeknumbers.iso ? "isoWeeknumber" : "weeknumber";
    });
    const onClick = (ev, weeknumber) => {
      const days = props.days.filter((day) => {
        return day[weeknumbersKey.value] === weeknumber;
      });
      emit("weeknumberclick", { weeknumber, days }, ev);
    };
    expose({ callSlot, onClick, daysInWeek: daysInWeek2, weekdayLabels, weeknumbersKey });
    return { emit, props, callSlot, daysInWeek: daysInWeek2, weekdayLabels, weeknumbersKey, onClick };
  }
}));
const __default__$3 = {
  inheritAttrs: false,
  render() {
    const title2 = h("div", __spreadValues({ class: "vc-title" }, this.navEvents), this.callSlot("header-title", this.page, this.page.title));
    const header = this.callSlot("header", this.page) || h("div", { class: ["vc-header", `align-${this.titlePosition}`] }, title2);
    const content = h(_sfc_main$6, { days: this.page.days, weeknumbers: this.options }, (day) => {
      return h(_sfc_main$7, __spreadProps(__spreadValues({}, this.$attrs), { day }), this.$slots);
    });
    return h("div", {
      class: [
        "vc-pane",
...
const _sfc_main$5 = /* @__PURE__ */ defineComponent(__spreadProps(__spreadValues({}, __default__$3), {
  ...
  props: {
    ...
    position: null,
    titlePosition: null,
    navVisibility: { default: () => getDefault("navVisibility") },
    row: null,
    rowFromEnd: null,
    column: null,
    columnFromEnd: null,
    showWeeknumbers: { type: [Boolean, null] },
    showIsoWeeknumbers: { type: [Boolean, null] }
  },
  setup(__props, { expose }) {
    ...
    const options = computed(() => {
      ...
      let side = false;
      let outside = false;
      const showNumbers = (_a = props.showWeeknumbers) != null ? _a : props.showIsoWeeknumbers;
      if (typeof showNumbers !== "string") {
        ...
        if (showNumbers.endsWith("outside")) {
          outside = side === "left" && props.column === 1 || side === "right" && props.columnFromEnd === 1;
        }
      }
    ...
    expose({ callSlot, navEvents, options });
    return { props, callSlot, navPopoverId, options, navPlacement, navEvents };
  }
}));
...
const _sfc_main$4 = defineComponent({
  ...
  setup(__props, { emit }) {
    const props = __props;
    const handler = (ev) => emit("push", ev);
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("span", {
        role: "button",
        class: normalizeClass([!props.enabled && "is-disabled"]),
        tabindex: props.enabled ? 0 : void 0,
        onClick: handler,
        onKeydown: _cache[0] || (_cache[0] = (ev) => unref(onSpaceOrEnter)(ev, handler))
      }, [
        renderSlot(_ctx.$slots, "default")
      ], 42, _hoisted_1$1);
    };
...
const _hoisted_1 = { class: "vc-nav-header" };
const _hoisted_2 = { class: "vc-nav-items" };
const _sfc_main$3 = defineComponent({
  name: "PopoverPanel",
  props: {
    items: null,
    enablePrev: { type: Boolean },
    enableNext: { type: Boolean },
    focusFirstItem: { type: Boolean }
  },
  emits: ["input", "focused", "panelchange", "titleclick"],
  setup(__props, { emit }) {
    const props = __props;
    const navContainer = ref(null);
    watch(props, async (n) => {
      var _a, _b, _c;
      if (n.focusFirstItem !== true)
        return;
      await nextTick();
      (_c = (_b = (_a = navContainer.value) == null ? void 0 : _a.querySelector(".vc-nav-item:not(.is-disabled)")) == null ? void 0 : _b.focus) == null ? void 0 : _c.call(_b);
      emit("focused");
    });
    const changePanel = (count) => {
      if (count === 0)
        return;
      if (count < 0 && !props.enablePrev)
        return;
      if (count > 0 && !props.enableNext)
        return;
      ...
      return openBlock(), createElementBlock("div", {
        ...
        createElementVNode("div", _hoisted_1, [
          createVNode(_sfc_main$4, {
            ...
            onPush: _cache[0] || (_cache[0] = () => changePanel(-1))
          }, {
            default: withCtx(() => [
              renderSlot(_ctx.$slots, "nav-left-button", {}, () => [
                createVNode(SvgIcon, {
          ...
          createVNode(_sfc_main$4, {
            ...
            onPush: _cache[1] || (_cache[1] = (ev) => emit("titleclick", ev))
          }, {
            default: withCtx(() => [
              renderSlot(_ctx.$slots, "nav-title")
            ]),
            _: 3
          }),
          createVNode(_sfc_main$4, {
            ...
            onPush: _cache[2] || (_cache[2] = () => changePanel(1))
          }, {
            default: withCtx(() => [
              renderSlot(_ctx.$slots, "nav-right-button", {}, () => [
                createVNode(SvgIcon, {
        ...
        createElementVNode("div", _hoisted_2, [
          renderSlot(_ctx.$slots, "nav-items", normalizeProps(guardReactiveProps({ items: __props.items })), () => [
            (openBlock(true), createElementBlock(Fragment, null, renderList(__props.items, (item) => {
              return openBlock(), createBlock(_sfc_main$4, {
                key: item.id,
                class: normalizeClass(["vc-nav-item", [
                  item.isActive && "is-active",
                  item.isCurrent && "is-current"
                ]]),
                enabled: item.isEnabled,
                "aria-label": item.ariaLabel,
                onPush: () => emit("input", item.value)
              }, {
                default: withCtx(() => [
                  createTextVNode(toDisplayString(item.label), 1)
                ]),
                _: 2
              }, 1032, ["class", "enabled", "aria-label", "onPush"]);
            }), 128))
          ])
        ])
      ], 512);
...
function useActivePanel(currentYear, filter) {
  return {
...
function useYearsFilter(yearRef, monthRef, validator, count = 12, offset = 0) {
  const { pageFromDate } = useCommons();
  return (year) => {
    const now = pageFromDate(new Date());
    const first = year - offset;
    const last2 = first + count;
    const items = [];
    for (let year2 = first; year2 < last2; year2++) {
      let enabled = false;
      for (let month = 1; month < 12; month++) {
        enabled = validator({ year: year2, month });
        if (enabled)
          break;
      }
      const str = year2.toString();
      items.push({
        id: str,
        label: str,
        ariaLabel: str,
        value: { year: year2, month: monthRef.value },
        isActive: year2 === yearRef.value,
        isCurrent: year2 === now.year,
        isEnabled: enabled
      });
    }
    return items;
...
function useMonthsFilter(yearRef, monthRef, validator) {
  const { locale, masks: masks2, pageFromDate } = useCommons();
  return (year) => {
    ...
    return locale.getMonthDates().map((date, month) => {
      ...
      return {
        ...
        label: locale.format(date, masks2.navMonths),
...
const __default__$2 = {
  render() {
    ...
    }, this.popoverEvents), __spreadProps(__spreadValues({}, this.$slots), {
      "nav-title": this.createSlot("month-panel-title", { items: this.activeItems, year: this.yearIndex }, this.yearIndex)
    }));
  }
};
const _sfc_main$2 = defineComponent(__spreadProps(__spreadValues({}, __default__$2), {
  name: "MonthPanel",
  props: {
    modelValue: null,
    validator: { type: Function, default: () => true }
  ...
  setup(__props, { expose, emit }) {
    ...
    const [focusFirstItem, focusFirst] = useRef(false, (n) => n != null ? n : false);
    ...
    return { createSlot, emit, props, year, month, yearIndex, changePanel, focusFirstItem, focusFirst, getMonthItems, activeItems, hasPrevPanel, hasNextPanel, handleInput, popoverEvents };
  }
}));
const __default__$1 = {
  render() {
    return h(_sfc_main$3, __spreadValues({
      ...
      "nav-title": this.createSlot("panel-title", { items: this.activeItems, year: this.yearIndex }, () => {
        if (this.monthMode)
          return this.yearIndex;
        const sorted = this.activeItems.map((item) => item.value.year).sort((a, b) => a - b);
        return `${sorted.at(0)} - ${sorted.at(-1)}`;
      })
    }));
  }
};
const _sfc_main$1 = defineComponent(__spreadProps(__spreadValues({}, __default__$1), {
  name: "MonthYearPanel",
  props: {
    modelValue: null,
    validator: { type: Function, default: () => true }
  ...
  setup(__props, { expose, emit }) {
    ...
    const {
      ...
      hasPrevPanel,
      hasNextPanel
    } = useActivePanel(yearIndex, (year2) => {
      return monthMode.value ? getMonthItems(year2) : getYearItems(year2);
    });
    const handleInput = (value) => {
      if (!monthMode.value) {
    ...
    return { createSlot, emit, props, year, month, monthMode, toggleMode, focusFirstItem, focusFirst, yearIndex, changePanel, getYearItems, getMonthItems, activeItems, hasPrevPanel, hasNextPanel, handleInput, popoverEvents };
  }
}));
const __default__ = {
  render() {
    return h(_sfc_main$3, __spreadValues({
      ...
      "nav-title": this.createSlot("year-panel-title", { items: this.activeItems, year: this.yearIndex }, () => {
        const sorted = this.activeItems.map((item) => item.value.year).sort((a, b) => a - b);
        return `${sorted.at(0)} - ${sorted.at(-1)}`;
      })
    }));
...
const _sfc_main = defineComponent(__spreadProps(__spreadValues({}, __default__), {
  name: "YearPanel",
  props: {
    modelValue: null,
    groupCount: { default: 9 },
    validator: { type: Function, default: () => true }
  },
  emits: ["update:modelValue"],
  setup(__props, { expose, emit }) {
    ...
    const getYearItems = useYearsFilter(year, month, props.validator, props.groupCount, 4);
    const {
      activeItems,
      hasPrevPanel,
      hasNextPanel
    ...
    return { createSlot, emit, props, year, month, focusFirstItem, focusFirst, yearIndex, changePanel, getYearItems, activeItems, hasPrevPanel, hasNextPanel, handleInput, popoverEvents };
  }
}));
const components = { Calendar: _sfc_main$b, DatePicker: _sfc_main$8, Popover: _sfc_main$h, PopoverRow };
...
var index = { install };
export { _sfc_main$b as Calendar, _sfc_main$5 as CalendarPage, _sfc_main$8 as DatePicker, _sfc_main$2 as MonthPanel, _sfc_main$1 as MonthYearPanel, _sfc_main$h as Popover, _sfc_main$3 as PopoverPanel, PopoverRow, screensPlugin as Screens, setup as SetupCalendar, _sfc_main as YearPanel, index as default };
